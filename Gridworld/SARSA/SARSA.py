...
for e in range(exp):
    ...
    # Initializing table for Q values
    ...
    for ep in range(episodes):
        ...
        for t in range(500):
            ...
            # SARSA update
            Q[x][y][a] = Q[x][y][a] + alpha*(r + gamma*(Q[x1][y1][a1]) - Q[x][y][a])
            x, y = s1
            a = a1
            tot_r += r
            if epsmin < epsilon:
...
fig.suptitle('SARSA: Average over last %d independent runs' % exp)
